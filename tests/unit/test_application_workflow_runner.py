...
def test_failed_case_stops_submissions_and_omits_raw_error(fixture_runner, capsys, failure_index):
    args, receipt, execute = fixture_runner
...
def test_execute_observes_each_durable_attempt_after_one_submission(monkeypatch, case):
    ...
    monkeypatch.setattr(runner, "validate_task_status_payload", lambda *_a, **_k: case.states[-1])
    monkeypatch.setattr(
        runner, "qualification_admin_session", lambda: nullcontext("fixture-cookie")
    )
    graph = Mock(return_value={"observed": True})
    monkeypatch.setattr(runner, "read_full_workflow_graph", graph)
    responses = [(200, b"{}"), (302, b""), (401, b"")]
