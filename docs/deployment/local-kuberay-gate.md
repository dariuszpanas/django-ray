...
Dockerfiles have deny-by-default context allowlists. Recursive local-environment, SQLite database,
and SQLite sidecar exclusions follow the broad source-directory exceptions so untracked nested
`.env`, `.env.*`, `*.sqlite3`, and `*.sqlite3-*` files are not restored to either image context.
The same one-time archive is the only source for the rendered Kubernetes manifests. The separately
archived v0.4.0 handoff baseline is
...
|---|---|---|---|
| Co-resident overlay, shared KubeRay quota or values, pinned operator install, or the data-preserving co-resident switch target | Required exact co-resident transition and authenticated task smoke, followed separately by the guarded full KubeRay gate | `required` | The co-resident run proves the five-pod, 1.8-CPU combined quota and internal-only service boundary. The ordinary gate still proves that the authoritative full local profile, setup, RuntimeEnv, API/task path, and cold Ray lifecycle remain intact. |
...
| Default-database enqueue routing and application receipt atomicity, with no change to worker consumption, polling, submission or execution | Required PostgreSQL container proof; KubeRay not applicable | `skip` | Run all 16 installed-wheel cases in `qualification/transactions/receipts.yaml`, including independent writer/observer connections, commit/rollback and cleanup. The public `Transaction Qualification` workflow provides this evidence. It is path-selected and separate from `CI Gate`; reviewers must check its successful current-source result when this row applies. |
| Worker command, queue selection, application image, polling, or database-backed execution behavior beyond the enqueue-only row above | Required | `required` for cluster-mode submission changes; otherwise `skip` | Proves all task-manager Deployments reconnect and consume a real task. |
...
For the dormant task-target-binding exception, retain the exact SQLite and PostgreSQL binding
migration results plus the explicit guarded-KubeRay-not-applicable decision. Do not report this
database evidence as enqueue provenance, a legacy mapping, target capacity, claim authorization,
work placement, or blue/green handoff validation. Record that both parents are protected and that
activation still requires tested task and policy cleanup ordering.

For the dormant target-routing exception, retain the exact SQLite and PostgreSQL routing migration
and coordination results plus the explicit guarded-KubeRay-not-applicable decision. Do not report
this database evidence as task or binding provenance, a legacy mapping, a live attestation, target
capacity, claim authorization, work placement, or blue/green handoff validation. Record that absent
route-selection provenance is unproved, both selection parents are protected, and cleanup deletes a
selection before either parent and every route revision before its route.

For the dormant worker-target-capability exception, retain the exact SQLite and PostgreSQL
capability migration and coordination results plus the explicit guarded-KubeRay-not-applicable
decision. Do not report this database evidence as a live worker advertisement, claim authorization,
pinned-work execution, Ray Job support, or blue/green handoff validation. Record that lease deletion
cascades the ephemeral current row, including through supported Admin inactive-lease cleanup; that
fail-closed withdrawal is the only indirect production mutation. CAS revision is not audit history,
...
workload pods with 3.3 CPU and 5,056 MiB requested and 9.8 CPU and 16,256 MiB
...
2. Builds `django-ray:<source-tag>` and `django-ray-worker:<source-tag>` from the same preflight
   ...
   argument to `Dockerfile.ray`. The legacy handoff fixture also replaces only its installed Ray
   distribution with the rendered cluster's exact version and runs `pip check`. Its generated
   Dockerfile retains the released application source and Dockerfile-specific context policy.
   This tests the released package's reader on the current Ray/Python tuple, not its original
   frozen dependency environment or a coordinated cluster upgrade. Ray rejects a 2.56 manager
   talking to a 2.58 cluster before application submission. The root `Dockerfile` and ordinary Compose path remain
...
15. Enqueues one fixed full-reporting order-fulfillment recovery showcase. The same durable task ID
    ...
    The single-attempt reservation-failure showcase must retain exactly 16 nodes and 21 edges:
    15 successful prerequisites and the failed bounded map. Its five downstream signatures were
    never submitted and must be absent, rather than fabricated as pending nodes. The successful
    showcase still requires all 21 nodes and 28 edges, including their indexed details and previews.
...
token manually into the page only in a trusted local session and confirm a protected action works.
Reload and confirm protected actions require another paste. Paste the token again, select **Forget
token**, and confirm protected actions still require a token before and after another reload. Do not
automate token retrieval into browser logs, screenshots, recordings, or artifacts, and do not include
the token in a URL.
