...
class _RayExecutor(_Executor):
    def __init__(self) -> None:
        ...
        self.task_execution_pk = get_current_task_execution_pk()
        self.progress_actor = None
        self.last_progress_json: str | None = None
    ...
    def submit_step(
        ...
        if self.progress_actor is not None:
            self.progress_actor.register.remote(node_id, label)
        options = {
        ...
        return self.remote_step.options(**options).remote(
    ...
    def _flush_progress(
        ...
        *,
        ...
        if force or serialized != self.last_progress_json:
            from django_ray.models import RayTaskExecution

            RayTaskExecution.objects.filter(pk=self.task_execution_pk).update(
                progress_data=serialized
            )
            self.last_progress_json = serialized
        return snapshot
    ...
    def finish_progress(self, *, failed: bool = False) -> None:
        ...
        for _ in range(10):
            snapshot = self._flush_progress(force=True, failed=failed)
            if snapshot is None:
                return
...
class WorkflowSignature(ABC):
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        try:
            result = executor.resolve(self._submit(executor, args, kwargs, "0"))
...
class Step(WorkflowSignature):
    ...
    def _submit(
        ...
        return executor.submit_step(self, input_args, input_kwargs, node_id)
...
class Group(WorkflowSignature):
    ...
    def _submit(
        ...
        return executor.collect(results)
...
class Map(WorkflowSignature):
    ...
    def _submit(
        ...
        results = [
            self.signature._submit(executor, (item,), {}, f"{node_id}.m{index}")
