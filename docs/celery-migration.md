...
| Ray | Ray 2.58.0 or a newer compatible release; align Ray and Python versions across task managers and Ray nodes |
...
| [`expires`](https://docs.celeryq.dev/en/stable/userguide/calling.html) | **Partial** | A django-ray backend alias may configure a snapshotted `QUEUE_TIMEOUT_SECONDS` policy, but Django 6 exposes no per-call expiry argument and django-ray does not accept Celery's `expires`. Keep Celery when each call needs a distinct expiry. See [Queue expiration](tasks.md#queue-expiration). |
...
| [Acknowledgement, redelivery, and worker-loss policy](https://docs.celeryq.dev/en/stable/userguide/tasks.html) | **Semantic rewrite** | django-ray uses database claims, worker leases, persisted Ray handles, and recovery rather than broker ACK/reject/requeue. Queued work can expire or be cancelled before application code runs, while work that starts may be replayed after uncertain completion. Use application idempotency keys. See [Delivery Semantics](architecture.md#delivery-semantics). |
