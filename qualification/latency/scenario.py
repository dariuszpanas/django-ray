"""Install the candidate offline and retain the bounded real-Job observations."""
...
from qualification.latency.contract import (
    DEFINITION_PATH,
    MAX_PROBE_BYTES,
    WORKLOAD,
    junit,
...
TARGET = Path("/tmp/django-ray-latency-qualification/target")
...
def execute(
    *,
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="job-latency-", dir=target.parent) as directory:
            ...
            if result.returncode != 0:
                raise wheel.QualificationError("latency-probe-failed")
        ...
        if isinstance(error, wheel.BoundedProcessError):
            wheel._emit_process_failure("latency-probe", error)
    ...
    if failure is None and not fixture_cleanup:
        failure = "latency-fixtures-not-cleaned"
    manifest = {
        "schema": "django-ray.latency-qualification",
        "schema_version": 1,
        "workload": WORKLOAD,
    ...
    if len(encoded) > 512 * 1024:
        raise wheel.QualificationError("latency-manifest-too-large")
...
if __name__ == "__main__":
    if len(sys.argv) != 1:
        raise SystemExit("latency qualification does not accept arguments")
