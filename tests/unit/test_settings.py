...
class TestValidateSettings:
    ...
    def test_workflow_graphs_have_no_pilot_default(self) -> None:
        assert "WORKFLOW_PROGRESS_SCHEMA_V3_PILOT" not in DEFAULTS

    @pytest.mark.parametrize("value", [False, True, 0, 1, "true", None])
    def test_retired_pilot_setting_requires_explicit_removal(self, value) -> None:
        with pytest.raises(ImproperlyConfigured, match="has been removed"):
            validate_settings(
                {"RAY_ADDRESS": "ray://localhost:10001", "WORKFLOW_PROGRESS_SCHEMA_V3_PILOT": value}
            )
